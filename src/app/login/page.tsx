...
import { setAuth, setAuthEmail } from "@/lib/auth";
...
function curvedPath(a: Pt, b: Pt, bendPx: number): string {
  ...
  return `M ${a.x.toFixed(2)} ${a.y.toFixed(2)} C ${c1.x.toFixed(2)} ${c1.y.toFixed(2)}, ${c2.x.toFixed(
    2
  )} ${c2.y.toFixed(2)}, ${b.x.toFixed(2)} ${b.y.toFixed(2)}`;
}

/**
 * Full-width flow layer that spans the entire hero content area.
 * Positions are your current baseline; arrows are now computed border-to-border.
 */
function HowItWorksFlowFull() {
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      // Border points (start on A border toward B, end on B border toward A)
      ...
      // Bend values tuned to your current layout
  ...
  return (
    <div ref={canvasRef} className="flow-canvas" aria-hidden="true">
      {/* SVG arrows layer (pixel-based viewBox so arrow ends match borders exactly) */}
      ...
      {/* Bubbles (positions unchanged from your baseline screenshot) */}
...
export default function LoginPage() {
  ...
  return (
    <div className="w-full overflow-x-hidden">
        ...
        <div className="relative h-[calc(100vh-72px)] w-full overflow-hidden">
          ...
          <div className="relative mx-auto h-full w-full max-w-[1200px] px-8 py-10">
            ...
            <div className="relative flex w-full items-start justify-between gap-10">
              ...
              {/* RIGHT login card (add space from header by pushing it down) */}
      ...
      <style jsx global>{`
        /* Remove vertical scrollbars on login page */
        html,
        body {
          height: 100%;
          overflow: hidden;
        ...
        /* Positions EXACTLY as your baseline screenshot */
        .flow-1 {
          left: 180px;
          top: 0px;
        }
